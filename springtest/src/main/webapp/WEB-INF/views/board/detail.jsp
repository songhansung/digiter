	...
	<div class="container">
	  ...
	  <c:if test="${board != null}">
		  <form action="">
		    ...
		    <a href="<%=request.getContextPath()%>/board/list?page=${cri.page}&search=${cri.search}&type=${cri.type}" class="list_but">
				<button type="button" class="btn btn-outline-dark">목록</button>
		  	</a>
		  	<c:if test="${user != null && user.id == board.writer}">
			  	<a href="<%=request.getContextPath()%>/board/modify?num=${board.num}" class="list_but">
					<button type="button" class="btn btn-outline-dark">수정</button>
			  	</a>
			  	<a href="<%=request.getContextPath()%>/board/delete?num=${board.num}" class="list_but">
